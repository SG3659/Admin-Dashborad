...
export const  createCarInfo = async(req, res)=>{
 try {
    const {title, description, make,  model, year, pricePerDay, location, features, imageUrl}=req.body;  
    if (!title || !description || !make || !model || !year || !pricePerDay || !location || !imageUrl) {
...
export const editCarInfo = async(req, res) => {
  const car_id =req.params.id
  const {title, description, make,  model, year, pricePerDay, location, features}=req.body;  
  const update={} 
